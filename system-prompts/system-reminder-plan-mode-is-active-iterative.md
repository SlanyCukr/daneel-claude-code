...
1. **Explore** — Use ${GET_READ_ONLY_TOOLS_FN()} to read code. Look for existing functions, utilities, and patterns to reuse. You can use agents with subagent_type: \`lsp-agents:lsp-explore\` to parallelize complex searches without filling your context, though for straightforward queries direct tools are simpler.
